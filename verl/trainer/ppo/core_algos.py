...
def compute_token_on_off_policy_loss(
    ...
    """LUFFY's token-level mixed on/off-policy objective.
    ...
    if on_policy_reshape == "no_reshape":
        ...
        constant = 0.1 if on_policy_reshape == "p_div_p_0.1" else 0.5
        prob = torch.exp(log_prob)
        old_prob = torch.exp(old_log_prob)
        ratio = (prob / (prob + constant)) / (old_prob / (old_prob + constant))
    else:
    ...
    if not loss_remove_clip:
        ...
        on_pg_loss = verl_F.masked_mean(on_pg_losses, (~prefix_mask) * eos_mask)
        on_pg_clipfrac = torch.zeros((), device=log_prob.device)
    ...
    if off_max_clip is not None:
        ...
        off_ratio_max_clip_frac = torch.zeros((), device=log_prob.device)
    ...
    if off_min_clip is not None:
        ...
        off_ratio_min_clip_frac = torch.zeros((), device=log_prob.device)

    off_ratio_mean = verl_F.masked_mean(off_ratio, prefix_mask * eos_mask)
    if off_ratio_mean.isnan().any().item():
        off_ratio_mean = torch.zeros((), device=log_prob.device)
    ...
    if off_pg_loss.isnan().item():
        off_pg_loss = torch.zeros((), device=log_prob.device)
    off_pg_clipfrac = torch.zeros((), device=log_prob.device)

    prefix_mask_float = prefix_mask.float()
    pg_losses = off_pg_losses * prefix_mask_float + on_pg_losses * (1 - prefix_mask_float)

    off_policy_probs = torch.exp(log_prob)
    off_policy_prob = verl_F.masked_mean(off_policy_probs, prefix_mask_float * eos_mask)
    if off_policy_prob.isnan().item():
        off_policy_prob = torch.zeros((), device=log_prob.device)
    on_policy_probs = torch.exp(old_log_prob)
    on_policy_prob = verl_F.masked_mean(on_policy_probs, (1.0 - prefix_mask_float) * eos_mask)
    if on_policy_prob.isnan().item():
        on_policy_prob = torch.zeros((), device=log_prob.device)

    if all_max_clip is not None:
        p_on_mask = (torch.exp(log_prob) <= all_max_clip).float()
